...
function slideLeft(){
	for(let r=0; r<rows; r++){
		...
		//line for animation
		//copy of content of the original row
		...
		for(let c=0; c<columns; c++){
            ...
            //line for animation
            //if current tile is not equal to the original tile apply animation
...
// Event listener to check where you touch the screen and prevent scrolling. Input targets any element that includes the word tile.
document.addEventListener("touchmove", (e) => {
    if (!e.target.className.includes("tile")) { // Corrected the class name check
        return;
    }
...
document.addEventListener("touchend", (e) => {
    ...
    // Set a threshold for swipe detection to prevent accidental swipes
    let threshold = 30; // Minimum swipe distance

    // Check if the swipe is horizontal or vertical, ensuring it's above the threshold
    ...
    // Generate a new tile (assuming setTwo is your function for adding a new tile)
    setTwo();
